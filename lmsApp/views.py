...
from lmsApp.models import Borrow
...
def save_user(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(
            'WEL COME TO Library',
            f'Hi ! {firstname} {lastname},your account has been successfully registered in Library by admin  now you can login your \n account  \n \n your username is:- {username} \n \nPassword is:- {password}\n \n Thankyou\n' ,
...
def save_borrow(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Enroll =form.cleaned_data.get("student")
            bdate = form.cleaned_data.get("borrowing_date")
            rdate = form.cleaned_data.get("return_date")
            book = form.cleaned_data.get("book")
            status = form.cleaned_data.get("status")

            send_mail(
            'WEL COME TO MANUU Library',
            f'Hi ! {Enroll}, you are successfully {book} issu   now you  \n  \n your borrowing date  is:- {bdate}\n \n return date  is:- {rdate}\n \n status is :- {status} \n \n Thankyou\n' ,
            'abdulrab4546@gmail.com',
...
def print_detail(request,pid):
    vehicle = Borrow.objects.get(id=pid)
    d = {'vehicle': vehicle}
    return render(request,'view_borrow.html', d)